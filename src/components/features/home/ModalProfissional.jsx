// src/components/features/ProfessionalModal.jsx
// Responsabilidade: Exibir o Modal (pop-up) com todos os detalhes do profissional, 
// incluindo o sobre, a lista de avaliações dos clientes e o botão de WhatsApp.
...
export function ModalProfissional({ isOpen, onClose, prof, onWhatsApp }) {
  if (!prof) return null;
    ...
    <Modal isOpen={isOpen} onClose={onClose} title="Perfil do Profissional">
      <div className="pb-4">
        ...
        <div className="flex items-center gap-4 mb-6">
          <img src={prof.avatar} className="w-16 h-16 rounded-full border-2 border-gray-700 object-cover" alt="avatar" />
          <div>
            <Heading level={4}>{prof.name}</Heading>
            <Text variant="sm" className="text-orange-400 font-bold uppercase text-xs tracking-wider">{prof.profession}</Text>
            {prof.verified && (
        ...
        <div className="flex justify-between items-center bg-gray-800 p-4 rounded-2xl mb-6 border border-gray-700 shadow-inner">
          <div>
            ...
            <div className="flex items-end gap-1">
              <span className="text-4xl font-extrabold text-white leading-none">{prof.rating.toFixed(1)}</span>
              <span className="text-sm font-bold text-gray-500 mb-1">/ 10</span>
            </div>
          </div>
          <div className="flex gap-1">
            {[...Array(5)].map((_, i) => (
              <Star key={i} className={`w-7 h-7 ${i < Math.round(prof.rating / 2) ? 'fill-yellow-500 text-yellow-500' : 'text-gray-600'}`} />
            ))}
          </div>
        </div>
        ...
        <div className="mb-6 px-1">
          ...
          <Text variant="sm" className="leading-relaxed text-gray-300">{prof.about}</Text>
        </div>
        ...
        <div className="space-y-3 mb-8">
          {prof.reviews.length > 0 ? prof.reviews.map((rev, index) => (
        ...
        {/* Botão de Contato */}
        <Button onClick={() => onWhatsApp(prof)} variant="primary" className="py-4 text-lg w-full shadow-2xl shadow-orange-500/20 gap-2">
