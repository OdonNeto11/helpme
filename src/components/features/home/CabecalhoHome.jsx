// src/components/features/home/CabecalhoHome.jsx
import { LogOut, User } from 'lucide-react';
import { Logo } from '../../ui/Logo';
import { Heading, Text } from '../../ui/Typography';

export function CabecalhoHome({ avatarUsuario, onResetar, onNavegarPerfil, onSair }) {
  return (
    <header className="w-full bg-gray-900 border-b border-gray-800 sticky top-0 z-50 pt-4 pb-4 shadow-lg shadow-black/50">
      <div className="max-w-md mx-auto px-4 flex items-center justify-between gap-3">
        <div className="flex items-center gap-3 cursor-pointer" onClick={onResetar}>
        ...
        <div className="flex items-center gap-1">
          {/* AQUI: O botão do perfil aciona o onNavegarPerfil */}
          <button 
            onClick={onNavegarPerfil} 
            className="p-2 text-gray-400 hover:text-orange-500 transition-colors rounded-full hover:bg-gray-800 flex items-center justify-center w-9 h-9 overflow-hidden"
          >
            {avatarUsuario ? (
              <img src={avatarUsuario} alt="Meu Perfil" className="w-full h-full object-cover" />
            ) : (
              <User className="w-5 h-5" />
            )}
          </button>
          
          {/* AQUI: O botão de sair aciona o onSair */}
          <button 
            onClick={onSair} 
